...
export default function TopDownMap(props: TopDownMapProps) {
  ...
  useEffect(() => {
    ...
    if (useStreamingMapCloud && mapMeta && props.manifest) {
      const nextGrid = projectLiveCloudToGrid(useStreamingMapCloud, props.manifest, mapMeta);
      setBaseGrid(nextGrid);
      if (!playbackViewportFitRef.current) {
  ...
  }, [mapMeta, props.mapPointCloud, props.manifest, props.streamProfile]);
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      drawMap(context, canvas, effectiveGrid, viewport, props, draftObstacle, draftErase);
    };
  ...
  }, [effectiveGrid, props, viewport, draftMeasurement, draftObstacle, draftErase]);
...
function createEmptyGrid(manifest: SceneManifest, mapMeta: OccupancyMapMeta): OccupancyGrid {
  return createGrid({
...
function projectPointsToGrid(
  ...
  const autoBounds = derivePointBounds(points, obstacleBand);
...
function projectLiveCloudToGrid(
  ...
  const points = decodedCloudToPoints(cloud);
  return projectPointsToGrid(points, manifest, mapMeta);
}

function decodedCloudToPoints(cloud: DecodedPointCloud): Point3[] {
  const points: Point3[] = [];
...
function drawTrajectory(context: CanvasRenderingContext2D, grid: OccupancyGrid, trajectory: Pose3D[]): void {
  ...
  context.strokeStyle = "#1d7f4f";
  context.lineWidth = Math.max(1.25 / context.getTransform().a, 0.8);
  context.beginPath();
