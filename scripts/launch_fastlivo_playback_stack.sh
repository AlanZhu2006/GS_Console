...
PLAYBACK_CONTAINER="${2:-fastlivo-playback}"
ROSBRIDGE_PORT="${3:-9090}"
WEB_PORT="${4:-5173}"
CONTROL_PORT="${5:-8765}"

bash "$ROOT_DIR/scripts/launch_fastlivo_playback.sh" "$BAG_PATH" "$PLAYBACK_CONTAINER"
bash "$ROOT_DIR/scripts/launch_playback_control_server.sh" "$BAG_PATH" "$PLAYBACK_CONTAINER" "$CONTROL_PORT"
